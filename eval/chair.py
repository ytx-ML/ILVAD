...
class CHAIR(object):
    def __init__(self, imids, coco_path):
        ...
        # read in synonyms
        synonyms = open("eval/synonyms.txt").readlines()
...
def print_metrics(hallucination_cap_dict, quiet=False):
    ...
    bleu = np.array([sentence_metrics[f"Bleu_{i}"] for i in range(1,5)])
    bleu = np.exp(np.mean(np.log(bleu)))
    ...
    if not quiet:
        print("METEOR\tCIDEr\tCHAIRs\tCHAIRi\tBleu")
        print(metric_string)
        return "METEOR\tCIDEr\tCHAIRs\tCHAIRi\tBleu\n" + metric_string
    else:
        return "METEOR\tCIDEr\tCHAIRs\tCHAIRi\tBleu\n" + metric_string
